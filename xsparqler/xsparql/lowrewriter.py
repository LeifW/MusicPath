...
def build_sparql_query(i, sparqlep, pfx, vars, from_iri, graphpattern, solutionmodifier, filterpattern):
    ...
    return '\nlet ' + query_aux(i) + ' := fn:concat(' + cnv_lst_str(dec_var, False) + ', "' + query + '")\n'


def build_for_loop(i, var):
    return 'for ' + query_result_aux(i) + ' at ' + position_var(i) + ' in mp:sparql(' + query_aux(i) + ')//_sparql_result:result\n'
